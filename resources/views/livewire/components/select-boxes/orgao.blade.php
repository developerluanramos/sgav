<div>
    <input type="hidden" name="orgaos" value="{{json_encode($orgaos)}}">
    ...
    <div class="relative">
        ...
        <input 
            ...
            placeholder="Pesquise Órgãos para adicionar"
    ...
    @foreach ($orgaos as $index => $funcao)
        <p>
            <span id="badge-{{$funcao['codigo_orgao']}}" class="inline-flex items-center px-2 py-1 me-2 text-sm font-medium text-blue-800 bg-blue-100 rounded-sm dark:bg-blue-900 dark:text-blue-300">
                {{$funcao['codigo_orgao']}} - {{$funcao['nome_orgao']}}
                <button wire:click="remove({{$index}})" type="button" class="inline-flex items-center p-1 ms-2 text-sm text-blue-400 bg-transparent rounded-xs hover:bg-blue-200 hover:text-blue-900 dark:hover:bg-blue-800 dark:hover:text-blue-300" data-dismiss-target="#badge-{{$funcao['codigo_orgao']}}" aria-label="Remove">
